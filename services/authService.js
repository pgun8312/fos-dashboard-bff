...
const authService = {
    signUp: async (req, res) => {
        ...
        const userEmailExists = await axios.post('http://localhost:8082/api/v1/users/get-userBy-email', {email}); 
        ...
        try {
            ...
            const createUserResponse = await axios.post('http://localhost:8082/api/v1/users', user);
            ...
            res.status(StatusCodes.CREATED).json({
                ...
                accountStatus: signUpResult.UserConfirmed === false ? "Unconfirmed" : "Confirmed",
    ...
    login:async (req, res) => {
        ...
        try{
            ...
            const userResponse = await axios.get(`${userServiceUrl}/users/${userSub}`);
    ...
    resetPasswordRequest: async (req, res) => {
        res.send("Used to request a password reset. Clients provide an email or username to initiate the password reset process");
    },
    resetPassword: async (req, res) => {
        res.send(" Used to confirm a password reset. Clients provide a confirmation code and a new password to reset the user's password");
    }
}
